...
#Usage "allthings.sh <Fwd_fastq.file.gz> <ref.list> <max_A> <max_B> <max_O> <#_processors>"
...
for ref in "${REF[@]}"; do
#Summary setup
echo "File        A        B       O       Reference      Total   QC      Mapped  Paired        Q60" > $ref.bwa_mapping_summary.txt
echo "File A B O Mean Median Min Max" > $ref.bedcov_len_summary.txt
echo "File A B O Mean Median Min Max" > $ref.bedcov_cov_summary.txt
for i in $(seq 1 $ALPHA); do
for j in $(seq 1 $BETA); do
for k in $(seq 1 $GAMMA); do
echo "mapping $var $i$j$k $ref"
#Mapping
bwa mem $ref $var.R1.fq.gz $var.R2.fq.gz -L 20,5 -t $DELTA -a -M -T 10 -A $i -B $j -O $k -R "@RG\tID:$var\tSM:$var\tPL:Illumina" 2> $LOG/bwa.$var.$ref.$i.$j.$k.log | mawk '$6 !~/[2-9].[SH]/ && $6 !~ /[1-9][0-9].[SH]/' | samtools view -@$DELTA -q 1 -SbT $ref - > $BAM/$var.$ref.$i.$j.$k.bam 2>$LOG/$var.$ref.$i.$j.$k.bam.log
...
echo "$var        $i    $j      $k      $ref      $TOT    $QC     $MAP    $PAIR $Q" >> $ref.bwa_mapping_summary.txt
